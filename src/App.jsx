...
function Toast({ msg, ok }) {
  return (
    <div style={{
      ...
      fontSize:14, zIndex:9999, boxShadow:'0 4px 24px #0009', whiteSpace:'nowrap',
    }}>{msg}</div>
  );
}
...
function TabRanking({ records, loading }) {
  ...
  const totalPts   = rank.reduce((s,a) => s+a.pts, 0);
  const metaTotal  = ASSESSORS.length * META.R1;
  const atingimento = metaTotal > 0 ? Math.round((totalR1 / metaTotal) * 100) : 0;

  // ordem do podio: 2o, 1o, 3o, 4o, 5o
  const podiumOrder  = [1, 0, 2];
  const podiumHeight = [180, 240, 150]; // altura da coluna
  const podiumAvSize = [72, 96, 64];
  const podiumRing   = [GOLD, GOLD, GOLD];

  if (loading && records.length === 0) {
    return <div style={{ textAlign:'center', padding:80, color:'#555' }}>Carregando...</div>;
  }
    ...
    <div style={{ maxWidth:760, margin:'0 auto', padding:'24px 0' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8, marginBottom:32 }}>
        {[
          ...
          <div key={m.label} style={{ ...CARD, textAlign:'center', padding:'20px 12px', border:'1px solid #1f1f2e' }}>
            <div style={{ color:'#555', fontSize:10, letterSpacing:2, marginBottom:10 }}>{m.label}</div>
            <div style={{ color:m.color, fontSize:38, fontWeight:800, lineHeight:1 }}>{m.val}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ textAlign:'center', marginBottom:32 }}>
        <span style={{ color:GOLD, fontSize:13, fontWeight:700, letterSpacing:3 }}>&#127942; TOP 5 ASSESSORES</span>
      </div>
            ...
            return (
              <div key={a.code} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', maxWidth:200 }}>
                ...
                <div style={{ textAlign:'center', marginBottom:10, padding:'0 8px' }}>
                  <div style={{
                    color: isFirst ? GOLD : '#e5e7eb',
                    fontWeight:800, fontSize: isFirst ? 14 : 12,
                ...
                <div style={{ position:'relative', marginBottom:0 }}>
                  ...
                  <div style={{
                    ...
                    background: ringCol, color:'#000', borderRadius:'50%',
                ...
                {/* coluna do podio */}
                <div style={{
                  marginTop:16, width:'100%', height:colH,
                  background:'linear-gradient(180deg, #1a1a2e 0%, #0f0f18 100%)',
                  border:'1px solid '+(isFirst ? GOLD+'44' : '#2a2a3e'),
                  borderBottom:'none', borderRadius:'8px 8px 0 0',
                  display:'flex', flexDirection:'column', alignItems:'center',
                  justifyContent:'center', padding:'16px 8px',
                  boxShadow: isFirst ? '0 -4px 20px '+GOLD+'22' : 'none',
                }}>
            ...
            <div key={a.code} style={{ ...CARD, display:'flex', alignItems:'center', gap:12 }}>
              ...
              <div style={{ textAlign:'right', flexShrink:0 }}>
                <div style={{ display:'flex', gap:10, fontSize:13 }}>
                  <span style={{ color:'#aaa' }}>R1<span style={{ color:'#e5e7eb', fontWeight:800, marginLeft:4 }}>{a.R1}</span></span>
                  <span style={{ color:'#aaa' }}>R2<span style={{ color:'#e5e7eb', fontWeight:800, marginLeft:4 }}>{a.R2}</span></span>
                </div>
      ...
      {/* TABELA COMPLETA */}
      <details style={CARD}>
        <summary style={{ color:'#555', fontSize:12, cursor:'pointer', padding:'4px 0', letterSpacing:1 }}>VER TODOS OS {rank.length} ASSESSORES</summary>
...
function TabAoVivo({ records, countdown, lastUpdate, load, loading, error }) {
  const recent = [...records].sort((a,b) => b.ts - a.ts).slice(0, 40);
  ...
  return (
    <div style={{ maxWidth:420, margin:'0 auto', padding:'40px 0' }}>
      <div style={{ textAlign:'center', marginBottom:32 }}>
        ...
        <h1 style={{ margin:'8px 0 4px', color:GOLD, fontSize:24, letterSpacing:3, fontWeight:800 }}>INSURANCE DAY</h1>
      ...
      <div style={CARD}>
        ...
        <input
          ...
          style={{
            ...
            border:'1px solid #1f1f2e', background:'#080810',
        ...
        {list.length > 0 && (
          <div style={{ marginTop:4, borderRadius:8, border:'1px solid #1a1a2e', background:'#080810', overflow:'hidden' }}>
            {list.map(a => (
              <button key={a.code} onClick={() => pick(a)} style={{
        ...
        <button onClick={go} style={{
          ...
          color:'#000', fontWeight:800, fontSize:15, cursor:'pointer', letterSpacing:2,
        }}>ENTRAR →</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth:500, margin:'0 auto', padding:'24px 0' }}>
      ...
      <div style={{ display:'flex', gap:12, marginBottom:16 }}>
        {BTNS.map(b => (
          <button key={b.type} disabled={saving} onClick={() => reg(b.type)} style={{
          ...
          {TABS.map(t => (
            <button
              ...
              style={{
                ...
                fontWeight:700, fontSize:12, letterSpacing:1, transition:'all .15s',
