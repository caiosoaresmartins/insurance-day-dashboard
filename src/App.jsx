...
// ====== FIREWORKS ENGINE (puro canvas, sem libs) ======
const fireworksRef = { canvas: null, ctx: null, particles: [], rockets: [], raf: null, active: false };

function initFireworks() {
  if (fireworksRef.canvas) return;
  const canvas = document.createElement('canvas');
  canvas.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:8888;';
  canvas.width  = window.innerWidth;
  canvas.height = window.innerHeight;
  document.body.appendChild(canvas);
  fireworksRef.canvas = canvas;
  fireworksRef.ctx    = canvas.getContext('2d');
...
function launchFireworks(type) {
  initFireworks();
  const { canvas, ctx } = fireworksRef;
  const W = canvas.width, H = canvas.height;
  // cores por tipo de acao
  const palettes = {
    R1:    ['#3b82f6','#60a5fa','#bfdbfe','#ffffff'],
    R2:    ['#eab308','#facc15','#fde047','#ffffff'],
    Venda: ['#d4af37','#f59e0b','#10b981','#ffffff','#f43f5e'],
  };
  ...
  for (let b = 0; b < bursts; b++) {
    setTimeout(() => {
      ...
      const count = type === 'Venda' ? 120 : 80;
      for (let i = 0; i < count; i++) {
        const angle  = (Math.PI * 2 * i) / count + Math.random() * 0.3;
        const speed  = 2 + Math.random() * 6;
        fireworksRef.particles.push({
          x, y,
  ...
  if (!fireworksRef.active) {
    fireworksRef.active = true;
    function tick() {
      ctx.fillStyle = 'rgba(4,4,15,0.18)';
      ctx.fillRect(0, 0, W, H);
      fireworksRef.particles = fireworksRef.particles.filter(p => p.alpha > 0.02);
      fireworksRef.particles.forEach(p => {
      ...
      if (fireworksRef.particles.length > 0) {
        fireworksRef.raf = requestAnimationFrame(tick);
      } else {
        ctx.clearRect(0, 0, W, H);
        fireworksRef.active = false;
      }
    }
    tick();
  }
...
async function kvAdd(user, type) {
  const r = await fetch('/api/kv', {
    ...
    body: JSON.stringify({ action:'add', record:{ code:user.code, name:user.name, squad:user.squad, type } }),
  });
...
function GalaxyBg() {
  ...
  useEffect(() => {
    ...
    const nebulas = [
      ...
      {x:W*0.50,y:H*0.30,rx:260,ry:130,col:'rgba(29,78,216,0.06)'},
      {x:W*0.25,y:H*0.55,rx:150,ry:80, col:'rgba(99,51,180,0.04)'},
      {x:W*0.75,y:H*0.45,rx:200,ry:100,col:'rgba(212,175,55,0.04)'},
    ];
    let raf;
    function draw() {
      ctx.clearRect(0,0,W,H);
...
function Av({ name, size=56, ring=GOLD, glow=false }) {
  ...
  return (
    <div style={{width:size,height:size,borderRadius:'50%',background:bg,border:'3px solid '+ring,display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontWeight:800,fontSize:size*0.32,flexShrink:0,letterSpacing:1,boxShadow:glow?'0 0 24px '+ring+'99,0 0 48px '+ring+'44':'0 0 8px '+ring+'33'}}>
      {initials(name)}
    </div>
  );
}

// Toast com animacao de entrada
function Toast({ msg, ok, pts }) {
  const [visible, setVisible] = useState(false);
  useEffect(() => { setTimeout(()=>setVisible(true),10); }, []);
  return (
    <div style={{
      position:'fixed', bottom:32, left:'50%', transform: visible ? 'translateX(-50%) translateY(0) scale(1)' : 'translateX(-50%) translateY(30px) scale(0.8)',
      background: ok ? 'linear-gradient(135deg,#065f46,#10b981)' : '#ef4444',
      color:'#fff', padding:'14px 32px', borderRadius:16, fontWeight:800,
      fontSize:16, zIndex:9999, boxShadow:'0 8px 40px #000a', whiteSpace:'nowrap',
...
// Animacao de número subindo
function FloatingPts({ pts, color }) {
  const [pos, setPos] = useState(0);
  const [op,  setOp]  = useState(1);
  useEffect(() => {
    setTimeout(()=>{ setPos(-60); setOp(0); }, 50);
  }, []);
  return (
    <div style={{
      ...
      opacity:op, transition:'all 1.2s ease-out',
...
function TabRanking({ records, loading }) {
  ...
  // detecta mudanca de posicao para animar
  useEffect(() => {
    if (prevRank.current.length > 0) setAnimate(true);
    prevRank.current = rank.map(a=>a.code);
    if (animate) setTimeout(()=>setAnimate(false), 800);
  }, [records]);
  ...
  const ating      = metaTotal>0 ? Math.round((totalR1/metaTotal)*100) : 0;
    ...
    <div style={{maxWidth:760,margin:'0 auto',padding:'24px 0'}}>
      ...
      <div style={{position:'relative',background:'linear-gradient(180deg,rgba(5,5,20,0.97) 0%,rgba(10,10,30,0.99) 100%)',borderRadius:20,padding:'40px 16px 0',marginBottom:24,minHeight:340,overflow:'hidden',border:'1px solid rgba(212,175,55,0.15)',boxShadow:'0 0 60px rgba(99,51,180,0.15)'}}>
        ...
        <div style={{display:'flex',alignItems:'flex-end',justifyContent:'center',gap:0,position:'relative',zIndex:2}}>
          {podiumOrder.map((idx,vi)=>{
            const a = top5[idx];
            if (!a) return <div key={vi} style={{flex:1}} />;
            const isFirst = idx===0;
            const avSize  = podiumAvSize[vi];
            const colH    = podiumHeight[vi];
            const ringCol = idx===0?GOLD:idx===1?'#c0c0c0':'#cd7f32';
            const pr      = premio(a);
            return (
              <div key={a.code} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',maxWidth:200,transition:'transform .5s ease',transform:animate?'scale(1.02)':'scale(1)'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:24}}>
        {[3,4].map(idx=>{
          ...
          if (!a) return null;
          return (
            <div key={a.code} style={{...CARD,display:'flex',alignItems:'center',gap:12,transition:'transform .2s'}} onMouseEnter={e=>e.currentTarget.style.transform='translateX(4px)'} onMouseLeave={e=>e.currentTarget.style.transform=''}>
              ...
              <div style={{textAlign:'right',flexShrink:0}}>
                ...
                <div style={{color:GOLD,fontWeight:800,fontSize:14,marginTop:4}}>{a.pts} pts</div>
              </div>
            </div>
          );
        })}
        ...
        {showAll && (
          <div style={{marginTop:14}}>
            {rank.slice(5).map((a,i)=>(
              <div key={a.code} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 0',borderBottom:'1px solid #1a1a2e',transition:'background .15s',borderRadius:6}} onMouseEnter={e=>e.currentTarget.style.background='rgba(212,175,55,0.04)'} onMouseLeave={e=>e.currentTarget.style.background=''}>
                ...
                <div style={{color:GOLD,fontWeight:700,fontSize:13,minWidth:52,textAlign:'right'}}>{a.pts} pts</div>
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <div style={{maxWidth:580,margin:'0 auto',padding:'24px 0'}}>
      ...
      {recent.map((r,i)=>{
        ...
        const isNew = i===0;
        return (
          <div key={r.id||i} style={{...CARD,marginBottom:8,display:'flex',alignItems:'center',gap:12,animation:isNew?'slideIn 0.4s ease-out':'none',borderColor:isNew?typeColor+'44':'#1f1f3a',boxShadow:isNew?'0 0 20px '+typeColor+'22':'none'}}>
            ...
            <span style={{background:typeColor+'22',border:'1px solid '+typeColor,color:typeColor,borderRadius:99,padding:'3px 12px',fontSize:12,fontWeight:700}}>{r.type}</span>
...
function TabRegistrar({ user, records, add, onLogout }) {
  ...
  const showToast = (msg,ok,pts) => {
    setToast({msg,ok,pts}); setTimeout(()=>setToast(null),3500);
  };
  ...
  const reg = async type => {
    ...
    try {
      ...
      // pts flutuando
      setFloatPts(PTS[type]);
      setTimeout(()=>setFloatPts(null), 1500);
  ...
  return (
    <div style={{maxWidth:500,margin:'0 auto',padding:'24px 0'}}>
      ...
      <div style={{...CARD,marginBottom:16}}>
        ...
        {[
          {type:'R1',val:myR1,max:4,color:'#3b82f6'},
          {type:'R2',val:myR2,max:4,color:'#eab308'},
          {type:'Venda',val:myV,max:2,color:'#10b981'},
        ].map(m=>(
