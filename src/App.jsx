...
const GOLD = '#d4af37';
const BG   = '#04040f';
const CAROUSEL_INTERVAL = 4500; // ms por slide
...
function launchFireworks(type) {
  ...
  for (let b=0;b<bursts;b++) {
    setTimeout(()=>{
      const x=W*(0.2+Math.random()*0.6),y=H*(0.1+Math.random()*0.4),n=type==='Venda'?120:80;
      for(let i=0;i<n;i++){const angle=(Math.PI*2*i)/n+Math.random()*0.3,spd=2+Math.random()*6;fwRef.particles.push({x,y,vx:Math.cos(angle)*spd,vy:Math.sin(angle)*spd,alpha:1,decay:0.012+Math.random()*0.01,size:1.5+Math.random()*2,color:cols[Math.floor(Math.random()*cols.length)],trail:[]});}
    },b*220);
  }
  if(!fwRef.active){
    fwRef.active=true;
    function tick(){ctx.fillStyle='rgba(4,4,15,0.18)';ctx.fillRect(0,0,W,H);fwRef.particles=fwRef.particles.filter(p=>p.alpha>0.02);fwRef.particles.forEach(p=>{p.trail.push({x:p.x,y:p.y});if(p.trail.length>6)p.trail.shift();p.trail.forEach((t,i)=>{ctx.beginPath();ctx.arc(t.x,t.y,p.size*(i/p.trail.length)*0.6,0,Math.PI*2);ctx.fillStyle=p.color;ctx.globalAlpha=p.alpha*(i/p.trail.length)*0.4;ctx.fill();});ctx.beginPath();ctx.arc(p.x,p.y,p.size,0,Math.PI*2);ctx.fillStyle=p.color;ctx.globalAlpha=p.alpha;ctx.fill();ctx.globalAlpha=1;p.x+=p.vx;p.y+=p.vy;p.vy+=0.08;p.vx*=0.98;p.alpha-=p.decay;});if(fwRef.particles.length>0)fwRef.raf=requestAnimationFrame(tick);else{ctx.clearRect(0,0,W,H);fwRef.active=false;}}
    tick();
  }
}
...
function useCountUp(target,duration=600){
  ...
  useEffect(()=>{
    ...
    const step=ts=>{const p=Math.min((ts-t0)/duration,1),ease=1-Math.pow(1-p,3);setVal(Math.round(start+diff*ease));if(p<1)requestAnimationFrame(step);else prev.current=target;};
...
function useKV(){
  ...
  const load=useCallback(async(quiet=false)=>{
    ...
    try{const data=await kvFetch();setRecords(Array.isArray(data)?data:[]);setLastUpdate(new Date());setCountdown(TICK);}catch(e){setError(e.message);}finally{setLoading(false);}
  },[]);
  useEffect(()=>{load();},[load]);
...
function GalaxyBg(){
  ...
  useEffect(()=>{
    ...
    const ctx=cv.getContext('2d');let W=window.innerWidth,H=window.innerHeight;cv.width=W;cv.height=H;
    ...
    let raf;
    function draw(){ctx.clearRect(0,0,W,H);neb.forEach(n=>{const g=ctx.createRadialGradient(n.x,n.y,0,n.x,n.y,n.rx);g.addColorStop(0,n.col);g.addColorStop(1,'transparent');ctx.save();ctx.scale(1,n.ry/n.rx);ctx.beginPath();ctx.arc(n.x,n.y*(n.rx/n.ry),n.rx,0,Math.PI*2);ctx.fillStyle=g;ctx.fill();ctx.restore();});stars.forEach(s=>{s.a+=s.spd*s.dir;if(s.a>1||s.a<0.1)s.dir*=-1;ctx.beginPath();ctx.arc(s.x,s.y,s.r,0,Math.PI*2);ctx.fillStyle=s.col;ctx.globalAlpha=s.a;ctx.fill();ctx.globalAlpha=1;});raf=requestAnimationFrame(draw);}
    draw();
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'32px 24px',gap:16,animation:'carouselIn 0.5s cubic-bezier(0.34,1.2,0.64,1)'}}>
      ...
      <div style={{display:'flex',gap:24,background:'rgba(255,255,255,0.03)',borderRadius:16,padding:'16px 32px',border:'1px solid rgba(212,175,55,0.1)'}}>
        <div style={{textAlign:'center'}}>
          <div style={{color:'#3b82f6',fontSize:36,fontWeight:800,lineHeight:1}}>{a.R1}</div>
          <div style={{color:'#444',fontSize:10,letterSpacing:2,marginTop:4}}>R1</div>
        </div>
        <div style={{textAlign:'center'}}>
          <div style={{color:'#eab308',fontSize:36,fontWeight:800,lineHeight:1}}>{a.R2}</div>
          <div style={{color:'#444',fontSize:10,letterSpacing:2,marginTop:4}}>R2</div>
        </div>
        <div style={{textAlign:'center'}}>
          <div style={{color:'#10b981',fontSize:36,fontWeight:800,lineHeight:1}}>{a.Venda}</div>
          <div style={{color:'#444',fontSize:10,letterSpacing:2,marginTop:4}}>VENDA</div>
        </div>
      </div>
      ...
      <div style={{width:'100%',maxWidth:320}}>
        {[
          {type:'R1',val:a.R1,max:META.R1,color:'#3b82f6'},
          {type:'R2',val:a.R2,max:META.R2,color:'#eab308'},
          {type:'Venda',val:a.Venda,max:META.Venda,color:'#10b981'},
        ].map(m=>(
          <div key={m.type} style={{marginBottom:8}}>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}>
              <span style={{color:'#555',fontSize:11}}>{m.type}</span>
              <span style={{color:m.color,fontSize:11,fontWeight:700}}>{m.val}/{m.max}</span>
            </div>
            <div style={{background:'#1f1f3a',borderRadius:4,height:6,overflow:'hidden'}}>
              <div style={{width:Math.min(100,(m.val/m.max)*100)+'%',height:'100%',background:'linear-gradient(90deg,'+m.color+','+m.color+'cc)',borderRadius:4,transition:'width .8s cubic-bezier(0.34,1.56,0.64,1)',boxShadow:'0 0 8px '+m.color}}/>
            </div>
          </div>
        ))}
      </div>
...
// ====== PODIO VIEW (slide 0 do carrossel) ======
function PodiumView({top5,deltas,cardRefs}){
  const podiumOrder=[1,0,2],podiumH=[180,240,150],podiumAvSz=[72,96,64];
  return(
    <div style={{position:'relative',background:'linear-gradient(180deg,rgba(5,5,20,0.97) 0%,rgba(10,10,30,0.99) 100%)',borderRadius:20,padding:'40px 16px 0',minHeight:340,overflow:'hidden',border:'1px solid rgba(212,175,55,0.15)',boxShadow:'0 0 60px rgba(99,51,180,0.15)',animation:'carouselIn 0.5s cubic-bezier(0.34,1.2,0.64,1)'}}>
      {[...Array(30)].map((_,i)=>(<div key={i} style={{position:'absolute',left:(i*37%100)+'%',top:(i*53%60)+'%',width:i%5===0?2:1,height:i%5===0?2:1,borderRadius:'50%',background:i%3===0?GOLD:i%3===1?'#a78bfa':'#fff',opacity:0.3+(i%4)*0.15,animation:`starPulse ${2+i%3}s ease-in-out infinite`,animationDelay:(i*0.2)+'s'}}/>))}
            ...
            <div key={a.code} ref={el=>cardRefs.current[a.code]=el} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',maxWidth:200}}>
              ...
              <div style={{marginTop:16,width:'100%',height:colH,background:'linear-gradient(180deg,rgba(20,20,50,0.9) 0%,rgba(8,8,25,0.95) 100%)',border:'1px solid '+(isFirst?GOLD+'55':'rgba(212,175,55,0.1)'),borderBottom:'none',borderRadius:'8px 8px 0 0',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'16px 8px',boxShadow:isFirst?'0 -8px 30px '+GOLD+'22':'none'}}>
                <div style={{display:'flex',gap:20}}>
                  <div style={{textAlign:'center'}}><AnimPts pts={a.R1} style={{color:'#e5e7eb',fontSize:isFirst?36:28,fontWeight:800,lineHeight:1}}/><div style={{color:'#444',fontSize:9,letterSpacing:1,marginTop:4}}>R1</div></div>
                  <div style={{textAlign:'center'}}><AnimPts pts={a.R2} style={{color:'#e5e7eb',fontSize:isFirst?36:28,fontWeight:800,lineHeight:1}}/><div style={{color:'#444',fontSize:9,letterSpacing:1,marginTop:4}}>R2</div></div>
                  {a.Venda>0&&<div style={{textAlign:'center'}}><AnimPts pts={a.Venda} style={{color:'#10b981',fontSize:isFirst?36:28,fontWeight:800,lineHeight:1}}/><div style={{color:'#444',fontSize:9,letterSpacing:1,marginTop:4}}>VENDA</div></div>}
                </div>
...
// ====== TAB RANKING com CARROSSEL ======
function TabRanking({records,loading}){
  const [showAll,setShowAll]=useState(false);
  const rank=buildRanking(records);
  const top5=rank.slice(0,5);

  // Carrossel: slide 0 = podio, slides 1-5 = spotlight individual
  const TOTAL_SLIDES=1+Math.min(top5.length,5); // 1 podio + ate 5 individuais
  const [slide,setSlide]=useState(0);
  const [paused,setPaused]=useState(false);
  const [slideKey,setSlideKey]=useState(0); // força re-mount para animar entrada
  ...
  useEffect(()=>{
    ...
    if(prevOrder.length>0){
      const nd={};
      rank.forEach((a,ni)=>{const oi=prevOrder.indexOf(a.code);if(oi!==-1&&oi!==ni)nd[a.code]=oi-ni;});
      if(Object.keys(nd).length>0){setDeltas(nd);setTimeout(()=>setDeltas({}),3000);}
    }
    prevRankOrder.current=rank.map(a=>a.code);
  },[records]);

  useLayoutEffect(()=>{
    Object.keys(cardRefs.current).forEach(code=>{const el=cardRefs.current[code];if(el)snapshots.current[code]=el.getBoundingClientRect().top;});
  });

  useLayoutEffect(()=>{
    Object.keys(cardRefs.current).forEach(code=>{
      const el=cardRefs.current[code],prevTop=snapshots.current[code];
      if(!el||prevTop==null)return;
      const delta=prevTop-el.getBoundingClientRect().top;
      if(Math.abs(delta)>2){el.style.transition='none';el.style.transform=`translateY(${delta}px)`;requestAnimationFrame(()=>{el.style.transition='transform 0.55s cubic-bezier(0.25,0.46,0.45,0.94)';el.style.transform='translateY(0)';});}
    });
  },[records]);
  ...
  return(
    <div style={{maxWidth:760,margin:'0 auto',padding:'24px 0'}}>
          ...
          <div style={{display:'flex',gap:5}}>
            {Array.from({length:TOTAL_SLIDES}).map((_,i)=>(
              <button key={i} onClick={()=>{setSlide(i);setSlideKey(k=>k+1);setPaused(true);setTimeout(()=>setPaused(false),8000);}}
                style={{width:i===slide?20:8,height:8,borderRadius:4,border:'none',cursor:'pointer',background:i===slide?GOLD:'#2a2a3e',transition:'all .3s',padding:0}}/>
            ))}
          </div>
          {/* Play/Pause */}
          <button onClick={()=>setPaused(p=>!p)}
            style={{background:'#1f1f2e',border:'1px solid #2a2a3e',color:'#555',borderRadius:8,padding:'4px 10px',cursor:'pointer',fontSize:13,transition:'color .2s'}}
      ...
      {!paused&&<div style={{height:2,background:'#1f1f2e',borderRadius:2,marginBottom:16,overflow:'hidden'}}>
      ...
      <div style={{...CARD,padding:0,overflow:'hidden',minHeight:380,marginBottom:16}}>
        {/* Slide 0: Podio */}
        {slide===0&&(
          <PodiumView key={'podium-'+slideKey} top5={top5} deltas={deltas} cardRefs={cardRefs}/>
        )}
        {/* Slides 1-5: Spotlight individual */}
        {slide>0&&top5[slide-1]&&(
          <SpotlightCard key={'spot-'+slideKey} a={top5[slide-1]} rank={slide-1} deltas={deltas}/>
        )}
      </div>
      ...
      {recent.map((r,i)=>{
        ...
        return(
          <div key={r.id||i} style={{...CARD,marginBottom:8,display:'flex',alignItems:'center',gap:12,animation:isNew?'slideIn 0.4s ease-out':'none',borderColor:isNew?tc+'44':'#1f1f3a',boxShadow:isNew?'0 0 20px '+tc+'22':'none'}}>
            ...
            <div style={{flex:1}}><div style={{color:'#e5e7eb',fontWeight:700,fontSize:14,textTransform:'uppercase',letterSpacing:.5}}>{r.name}</div><div style={{color:'#555',fontSize:11}}>{r.squad} · {fmt(r.ts)}</div></div>
          ...
          <div style={{marginTop:4,borderRadius:8,border:'1px solid #1a1a3a',background:'rgba(4,4,20,0.95)',overflow:'hidden'}}>
            {list.map(a=>(
              <button key={a.code} onClick={()=>pick(a)} style={{width:'100%',textAlign:'left',padding:'10px 12px',border:'none',borderBottom:'1px solid #1a1a3a',background:'transparent',cursor:'pointer',display:'flex',alignItems:'center',gap:10}}>
                <Av name={a.name} size={32} ring={a.color}/>
                <div><div style={{color:'#e5e7eb',fontSize:13,fontWeight:600,textTransform:'uppercase',letterSpacing:.5}}>{a.name}</div><div style={{color:'#555',fontSize:11}}>{a.code} · {a.squad}</div></div>
              </button>
            ))}
          </div>
        )}
...
function TabRegistrar({user,records,add,onLogout}){
  ...
  const reg=async type=>{
    ...
    try{await add(user,type);launchFireworks(type);setFloatPts(PTS[type]);setTimeout(()=>setFloatPts(null),1500);showToast(type+' registrado com sucesso!',true,PTS[type]);}
    catch(e){showToast(e.message,false,null);}finally{setSaving(false);}
  };
    ...
    <div style={{maxWidth:500,margin:'0 auto',padding:'24px 0'}}>
      ...
      <div style={{...CARD,marginBottom:16,display:'flex',alignItems:'center',gap:14,justifyContent:'space-between'}}>
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          ...
          <div><div style={{color:'#e5e7eb',fontWeight:800,fontSize:16,textTransform:'uppercase',letterSpacing:1}}>{user.name}</div><div style={{color:'#555',fontSize:12}}>{user.squad} · {user.code}</div></div>
        </div>
      ...
      {mine.length>0&&(
        <div style={CARD}>
          ...
          {[...mine].sort((a,b)=>b.ts-a.ts).slice(0,8).map((r,i)=>{
            ...
            return(<div key={i} style={{display:'flex',justifyContent:'space-between',padding:'7px 0',borderBottom:'1px solid #1a1a2e',alignItems:'center',animation:i===0?'slideIn 0.4s ease-out':'none'}}><span style={{color:b?b.color:'#aaa',fontSize:13,fontWeight:700}}>{r.type}</span><span style={{color:'#444',fontSize:11}}>{fmt(r.ts)}</span><span style={{color:GOLD,fontWeight:800,fontSize:12}}>+{PTS[r.type]} pts</span></div>);
          })}
        </div>
      )}
    </div>
    ...
    <>
      ...
      <div style={{position:'relative',zIndex:1,padding:'0 16px',maxWidth:800,margin:'0 auto'}}>
        <div style={{padding:'14px 0 0',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:'1px solid rgba(212,175,55,0.15)'}}>
          ...
          {user&&(<div style={{display:'flex',alignItems:'center',gap:8}}><Av name={user.name} size={26} ring={user.color}/><span style={{color:'#888',fontSize:12,textTransform:'uppercase',letterSpacing:1}}>{user.name.split(' ')[0]}</span></div>)}
        </div>
