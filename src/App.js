...
const LS_KEY = "insurance_day_local_v1";
...
function burst(canvas, big = false) {
  if (!canvas) return;
...
function LoginScreen({ onLogin }) {
  ...
  const filtered = q.length >= 1
    ? ASSESSORS.filter(a => a.name.toLowerCase().includes(q.toLowerCase()) || a.code.toLowerCase().includes(q.toLowerCase())).slice(0,8)
    : [];
  function pick(a) { setSel(a); setQ(a.name); setErr(""); }
  function go() { if (!sel) { setErr("Selecione um assessor da lista"); return; } onLogin(sel); }
  return (
    <div style={S.page}>
      <style>{`* { box-sizing:border-box; font-family:system-ui,-apple-system,BlinkMacSystemFont,'Inter',sans-serif; } body{margin:0;background:#080808;} @keyframes pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.04)}} @keyframes fadeInUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}`}</style>
      <MeteorBg />
      <div style={{...S.card, maxWidth:440, position:"relative", zIndex:1}}>
        ...
        <input style={S.input} placeholder="Ex: A98943 ou Israel Gusso" value={q}
        ...
        {sel && (
          <div style={{...S.card, background:"#ffffff0a", border:`1px solid ${sel.squadColor}44`, padding:16, marginTop:16, display:"flex", alignItems:"center", gap:14}}>
            <Avatar name={sel.name} size={48} border={sel.squadColor} glow />
            <div>
...
function RegisterScreen({ user, records, onAdd, onGoRanking, onLogout }) {
  ...
  function showToast(msg, ok) { setToast({msg,ok}); setTimeout(()=>setToast(null),3000); }
  ...
  return (
    <div style={S.page}>
      <MeteorBg />
      <canvas ref={confRef} style={{position:"fixed",inset:0,zIndex:5,pointerEvents:"none",width:"100%",height:"100%"}}
      ...
      <div style={{position:"relative",zIndex:1,width:"100%",maxWidth:520,margin:"0 auto",padding:"24px 16px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <Avatar name={user.name} size={44} border={user.squadColor} glow />
            <div>
        ...
        <div style={{display:"flex",gap:12,marginBottom:20}}>
          {btnTypes.map(b=>(
            <button key={b.type} disabled={loading} onClick={()=>register(b.type)}
              style={{flex:1,padding:"20px 8px",borderRadius:16,background:`${b.color}22`,border:`2px solid ${b.color}`,
                color:"#fff",cursor:"pointer",transition:"all 0.2s",display:"flex",flexDirection:"column",
                alignItems:"center",gap:6,opacity:loading?0.6:1}}
        ...
        {myRecs.length > 0 && (
          <div style={S.card}>
            ...
            {[...myRecs].reverse().slice(0,5).map((r,i)=>{
              const b = btnTypes.find(b=>b.type===r.type);
              return (
                <div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid #ffffff0d"}}>
                  <span style={{color:b?.color,fontSize:13}}>{b?.icon} {r.type}</span>
...
function RankingScreen({ user, records, onGoRegister, onLogout }) {
  ...
  useEffect(()=>{ if(confRef.current && ranking[0]?.pts>0) burst(confRef.current, true); },[]);
...
export default function App() {
  ...
  function handleLogin(assessor) { setUser(assessor); setScreen("register"); }
  function handleAdd(updated) { setRecords(updated); saveRecords(updated); }
