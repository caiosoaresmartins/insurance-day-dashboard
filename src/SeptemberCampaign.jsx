...
function synthSting(){
  ...
  const now=ctx.currentTime;const freqs=[110,164.81,220,329.63,440];
...
function Intro({onClose}){
  ...
  return <div className="campaign-intro"><canvas ref={canvasRef}/><div className="campaign-vignette"/><button className="campaign-sound" onClick={()=>setMuted(v=>!v)}>{muted?'🔇 ATIVAR SOM':'🔊 SOM ATIVO'}</button><div className="campaign-intro-content">
    ...
    <h1 className={step>=2?'show':''}>O CEO<br/><span>ENDOIDOU.</span></h1>
    <p className={step>=3?'show':''}>Setembro começou. Três metas simples. Recompensa rápida. Comissão agressiva.</p>
    <div className={`campaign-reveal ${step>=4?'show':''}`}><div><b>R$ 100</b><span>3 REUNIÕES AGENDADAS</span></div><div><b>R$ 200</b><span>1 REUNIÃO REALIZADA</span></div><div className="gold"><b>50%</b><span>COMISSÃO NA VENDA</span></div></div>
