...
function AccessGate({onLogin}){
  ...
  return <main className="mseg-access-shell"><section className="mseg-access-card"><span className="mseg-kicker">SETEMBRO · MÊS DO SEGURO</span><h1>Acesse sua campanha</h1><p>Cada assessor visualiza apenas a própria produção e o próprio histórico.</p><div className="mseg-access-tabs"><button className={mode==='advisor'?'active':''} onClick={()=>setMode('advisor')}>ASSESSOR</button><button className={mode==='admin'?'active':''} onClick={()=>setMode('admin')}>AUDITOR</button></div><form onSubmit={submit}>{mode==='advisor'?<><label>CÓDIGO XP</label><input autoFocus value={code} onChange={e=>setCode(e.target.value)} placeholder="Ex.: A26305" autoComplete="off"/><small>Digite somente o seu código de assessor.</small></>:<><label>CREDENCIAL DE AUDITORIA</label><input type="password" value={secret} onChange={e=>setSecret(e.target.value)} placeholder="Senha administrativa" autoComplete="current-password"/><small>Acesso exclusivo para conferência e ajustes da campanha.</small></>}{error&&<div className="mseg-error">{error}</div>}<button className="mseg-access-submit" disabled={busy||(!code&&mode==='advisor')||(!secret&&mode==='admin')}>{busy?'VALIDANDO...':mode==='admin'?'ENTRAR NA AUDITORIA →':'ACESSAR MEU HISTÓRICO →'}</button></form></section></main>
}
...
function AuditorView({session,records,api,onLogout,countdown}){
  ...
  async function add(){setMsg('');try{await api({action:'add',record:{code:code.trim().toUpperCase(),type}});setMsg('Registro incluído pelo auditor e salvo no ledger.');}catch(e){setMsg(e.message)}}
